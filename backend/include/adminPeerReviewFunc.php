...
class adminPeerReviewFunc{
    ...
    final protected function __destruct(){		// destructor
    ...
    {
    	$filter="?filter[where][performanceinfoid]=$kpiid&filter[where][reviewforempid]=$empid"; //filter to obtain kpi ratings for employee
    ...
    public function retrieveKpiName($kpiid)
    {
    	...
    	$url='http://localhost:3001/api/Performanceinfos'.$filter; // creating api url with filters - using Performanceinfos table
    	return $url
    }
